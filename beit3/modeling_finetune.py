...
from beit3.modeling_utils import BEiT3Wrapper, _get_base_config, _get_large_config
...
class BEiT3ForCaptioning(BEiT3Wrapper):
    ...
    def forward(self, image, text_ids, padding_mask, language_masked_pos, text_len=None, incremental_state=None,
                **kwargs):
...
class BEiT3ForRetrieval(BEiT3Wrapper):
    def __init__(
        ...
        # self.vl_head = nn.Linear(embed_dim, embed_dim, bias=False)
        self.language_head.apply(self._init_weights)
        self.vision_head.apply(self._init_weights)
        # self.vl_head.apply(self._init_weights)
    ...
    def encode_image(self, image=None, **kwargs):
        if image is not None:
            outputs = self.beit3(
                textual_tokens=None,
                visual_tokens=image,
    ...
    def encode_text(self, text_description=None, padding_mask=None, **kwargs):
        if text_description is not None:
            outputs = self.beit3(
                textual_tokens=text_description,
                visual_tokens=None,
    ...
    def encode_vl(self, image=None, text_description=None, padding_mask=None, **kwargs):
        if image is not None and text_description is not None:
            outputs = self.beit3(
                textual_tokens=text_description,
                visual_tokens=image,
                text_padding_position=padding_mask,
            ...
            x = outputs["encoder_out"]
            x = x[:, 0, :]
            # vl_cls = self.vl_head(x)
            # vl_cls = x
            # vl_cls = F.normalize(vl_cls, dim=-1)
        # else:
        #     # vl_cls = None
        #     x = None
        return x
